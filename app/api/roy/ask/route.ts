...
export async function POST(req: Request) {
  try {
    ...
    if (!result.ok) {
      logChatServerError("roy", result.outcome === "context_failed" ? "context" : result.outcome === "model_failed" ? "openai" : "unexpected", new Error(`Workforce run ${result.runId} failed: ${result.outcome}`));
    ...
    logChatServerError("roy", "unexpected", error);
