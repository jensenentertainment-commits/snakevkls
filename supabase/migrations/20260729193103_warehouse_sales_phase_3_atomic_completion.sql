...
begin
  ...
  loop
    if jsonb_typeof(line) <> 'object'
      or jsonb_typeof(line -> 'productId') <> 'string'
      or (line ->> 'productId') !~
        '^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89aAbB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$'
      or jsonb_typeof(line -> 'quantity') <> 'number'
      or (line ->> 'quantity') !~ '^[0-9]+$'
      or (line ->> 'quantity')::bigint < 1
      or (line ->> 'quantity')::bigint > 2147483647
      or jsonb_typeof(line -> 'unitPriceMinor') <> 'number'
      or (line ->> 'unitPriceMinor') !~ '^[0-9]+$'
      or (line ->> 'unitPriceMinor')::numeric > 9223372036854775807
    then
  ...
  select
    jsonb_agg(
      jsonb_build_object(
        'product_id', (line ->> 'productId')::uuid,
        'quantity', (line ->> 'quantity')::integer,
        'unit_price_minor', (line ->> 'unitPriceMinor')::bigint
      )
  ...
  from jsonb_array_elements(requested_lines) as requested(line);
  ...
  for sale_line in
    ...
    where warehouse_line.sale_id = sale_id
  ...
  insert into public.warehouse_sale_shopify_sync_jobs (
    ...
    'snake://warehouse-sale/' || sale_id::text,
    outbox_payload,
    outbox_payload_hash
  )
