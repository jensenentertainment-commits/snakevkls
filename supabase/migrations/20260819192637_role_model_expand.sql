-- Role Model expand phase.
-- Keep the legacy `lager` value valid until production profiles have been
-- explicitly classified and migrated. A later contract migration removes it.
  ...
  select exists (
    ...
    where profile.id = (select auth.uid())
      ...
      and profile.role = any (
        array['admin', 'user', 'warehouse', 'lager']::text[]
      )
  );
$$;
...
using (
  id = (select auth.uid())
  and active is true
  and role in ('admin', 'user', 'warehouse', 'lager')
);

-- Products and collections previously trusted every authenticated session.
-- Require an active application profile as part of the role migration.
...
drop policy if exists "Authenticated read product collections"
...
-- Lager: user keeps today's operative rights; warehouse is equally operative.
drop policy if exists "Active users read inventory" on public.inventory;
drop policy if exists "Warehouse roles insert inventory" on public.inventory;
drop policy if exists "Warehouse roles update inventory" on public.inventory;
drop policy if exists "Warehouse roles delete inventory" on public.inventory;
create policy "Operational roles read inventory"
on public.inventory for select to authenticated
using ((select private.has_role(array['admin', 'user', 'warehouse', 'lager']::text[])));
create policy "Operational roles insert inventory"
on public.inventory for insert to authenticated
with check ((select private.has_role(array['admin', 'user', 'warehouse', 'lager']::text[])));
...
drop policy if exists "Warehouse roles insert locations" on public.locations;
drop policy if exists "Warehouse roles update locations" on public.locations;
...
drop policy if exists "Active users read zones" on public.zones;
...
drop policy if exists "Warehouse roles insert stock movements" on public.stock_movements;
...
drop policy if exists "Active users read activity log" on public.activity_log;
drop policy if exists "Active users insert activity log" on public.activity_log;
...
with check (
  (select private.has_role(array['admin', 'user', 'warehouse', 'lager']::text[]))
  and actor_id = (select auth.uid())
);

-- Viper remains available to ordinary users and the restricted warehouse role.
drop policy if exists "Active users read Viper orders" on public.orders;
drop policy if exists "Active users read Viper order lines" on public.order_lines;
drop policy if exists "Active users read Viper pick jobs" on public.pick_jobs;
drop policy if exists "Active users read Viper pick lines" on public.pick_lines;
drop policy if exists "Active users read Viper events" on public.viper_events;
drop policy if exists "Active users read Viper pick exceptions" on public.pick_exceptions;
...
-- Lagersalg belongs to admin/user. Legacy lager remains compatible only until
-- explicit profile migration; warehouse is intentionally excluded.
drop policy if exists "Active users read warehouse sales" on public.warehouse_sales;
drop policy if exists "Active users read warehouse sale lines" on public.warehouse_sale_lines;
drop policy if exists "Active users read warehouse sale Shopify jobs"
...
using ((select private.has_role(array['admin', 'user', 'lager']::text[])));
...
-- Grants are a separate Data API gate from RLS. Preserve only the reads the
-- browser/server clients use; service_role grants remain unchanged.
revoke all on table public.products, public.product_collections from anon;
revoke all on table public.products, public.product_collections from authenticated;
...
begin
  ...
  if requested_payment_method not in ('vipps', 'cash') then
  ...
  if not exists (
    ...
    where profile.id = requested_actor_id
      and profile.active is true
      and profile.role in ('admin', 'user', 'lager')
  ) then
